...
def set_game_limit():
    ...
    while (True):
        ...
        if (not (lower_limit.isdigit() and upper_limit.isdigit())):
            print("请输入数字！")
        elif (int(upper_limit) <= int(lower_limit)):
            print("上限必需大于下限！")
        elif (int(upper_limit) - int(lower_limit) < 4):
...
def start_game(answer, lower_limit, upper_limit, players):
    ...
    while(True):
        for player in players:
            ...
            # 检查号码和终极密码是不是一样
            if (is_guess_matched(guess, answer)):
                ...
                # 更新上下限
                lower_limit, upper_limit = update_limit(guess, answer, lower_limit, upper_limit)
...
def is_guess_valid(guess_input, lower_limit, upper_limit):
    ...
    3. 小于上限
    '''
    return guess_input.isdigit() and int(guess_input) > lower_limit and int(guess_input) < upper_limit
...
def show_winner(players):
    ...
    highest_points = max(players.values())
    print()
    print("===获胜玩家===")
    for player in players:
        if (players[player] == highest_points):
            print(player)
#endregion
